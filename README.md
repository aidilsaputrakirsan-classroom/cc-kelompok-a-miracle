...
## Docker Quick Start
Langkah ini untuk menjalankan backend di Docker dengan port host 8001 supaya tidak bentrok dengan service lain di port 8000.

1. Build image
```docker build -t cloudapp-backend:v1 .```

2. Jalankan container
```docker run --name cloudapp-backend-test -p 8001:8000 --env-file .env cloudapp-backend:v1```

3. Lihat log container
```docker logs -f cloudapp-backend-test```

4. Cek container aktif
```docker ps```

5. Stop container:
```docker stop cloudapp-backend-test```

6. Hapus container:
```docker rm cloudapp-backend-test```
...
```
1. Gejala
Port 8000 gagal dipakai saat menjalankan container baru.
Penyebab
Port 8000 sudah digunakan container lain.

Solusi
Jalankan container dengan mapping port host berbeda, misalnya -p 8001:8000, atau stop container lama.

2. Gejala
Tidak ada output saat menjalankan docker logs backend.
Penyebab
Nama container yang dipanggil tidak sesuai, atau belum ada request masuk.

Solusi
Gunakan nama container dari docker ps, lalu akses endpoint agar request log muncul.

3. Gejala
Swagger Authorize menampilkan Unprocessable Content.
Penyebab
Alur OAuth2 form di Swagger tidak sesuai dengan endpoint login yang menerima JSON email dan password.

Solusi
Login melalui endpoint POST /auth/login untuk mengambil token, lalu kirim token Bearer ke endpoint protected.

4. Gejala
GET /auth/me mengembalikan Not authenticated.
Penyebab
Header Authorization tidak dikirim atau token tidak valid.

Solusi
Login ulang untuk token baru, lalu kirim header Authorization: Bearer access_token.
```

## Testing Checklist

| No | Fitur/Uji | Endpoint | Method | Auth Required | Input/Prekondisi | Expected Status Code | Expected Hasil | Status |
|---|---|---|---|---|---|---|---|---|
| 1 | Health Check | `/health` | GET | Tidak | Server berjalan | 200 | API mengembalikan status healthy | ✅ |
| 2 | Register User Valid | `/auth/register` | POST | Tidak | Body: email unik, name valid, password kuat | 201 | User baru berhasil dibuat | ✅ |
| 3 | Register Email Duplikat | `/auth/register` | POST | Tidak | Body: email yang sudah terdaftar | 400 | Muncul pesan email sudah terdaftar | ✅ |
| 4 | Login Valid | `/auth/login` | POST | Tidak | Body: email + password benar | 200 | Mengembalikan access_token dan data user | ⬜ |
| 5 | Login Invalid | `/auth/login` | POST | Tidak | Body: email/password salah | 401 | Muncul pesan autentikasi gagal |  |
