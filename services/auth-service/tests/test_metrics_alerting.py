"""
Unit tests — error rate alerting di MetricsCollector (auth-service).
...
class TestNoAlert:
    def test_error_rate_exactly_at_threshold_no_alert(self):
        ...
        with patch.object(metrics_module, "logger") as mock_log:
            # 9 sukses + 1 error = 10 total, 10.0% — threshold adalah > 10%
            _record(mc, 9, 1)
    ...
    def test_too_few_requests_no_alert(self):
        ...
        mc = MetricsCollector()
        with patch.object(metrics_module, "logger") as mock_log:
            # Semua error tapi < ALERT_MIN_REQUESTS request
...
class TestAlertTriggered:
    ...
    def test_alert_fires_at_minimum_viable_rate(self):
        ...
        mc = MetricsCollector()
        with patch.object(metrics_module, "logger") as mock_log:
            _record(mc, 3, 2)  # 5 total, 40% error rate
            mock_log.critical.assert_called_once()
...
class TestCooldown:
    def test_second_alert_suppressed_within_cooldown(self):
        ...
        mc = MetricsCollector()
        with patch.object(metrics_module, "logger") as mock_log:
            _record(mc, 8, 2)   # alert pertama
            assert mock_log.critical.call_count == 1

            _record(mc, 0, 10)  # lebih banyak error dalam cooldown
            assert mock_log.critical.call_count == 1  # tetap 1
    ...
    def test_alert_fires_again_after_cooldown_expires(self):
        ...
        with patch.object(metrics_module, "logger") as mock_log:
            with patch("metrics.time") as mock_time:
                ...
                _record(mc, 8, 2)  # alert pertama di t=base
                assert mock_log.critical.call_count == 1

                # Lewati cooldown
                mock_time.time.return_value = base + ALERT_COOLDOWN_SECONDS + 1
                _record(mc, 8, 2)  # alert kedua setelah cooldown
...
class TestSlidingWindow:
    def test_old_requests_excluded_from_window(self):
        ...
        with patch("metrics.time") as mock_time:
            # t=0: catat 2 error (tapi terlalu sedikit untuk trigger alert)
            mock_time.time.return_value = base
            _record(mc, 0, 2)

            # t=window+1: entri lama sudah expired, tambah 5 request sukses
    ...
    def test_window_count_reflects_only_recent_requests(self):
        ...
        with patch("metrics.time") as mock_time:
            mock_time.time.return_value = base
            _record(mc, 10, 0)  # 10 request di t=0

            mock_time.time.return_value = base + ALERT_WINDOW_SECONDS + 5
            _record(mc, 3, 0)   # 3 request di t=window+5 (yang lama expired)
