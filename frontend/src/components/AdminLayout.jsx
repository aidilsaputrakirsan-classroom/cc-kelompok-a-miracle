...
const SidebarItem = ({ icon: Icon, label, path, active }) => (
  ...
  >
    <Icon className={cn("w-5 h-5", active ? "text-red-600" : "text-slate-400 group-hover:text-slate-600")} />
    <span>{label}</span>
  </Link>
);
    ...
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-white border-r border-slate-200 p-6 fixed h-full">
        ...
        <button 
          ...
          className="mt-auto flex items-center gap-3 px-4 py-3 rounded-xl text-slate-500 hover:bg-red-50 hover:text-red-600 transition-all duration-200"
        >
